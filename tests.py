...
class AppTests(unittest.TestCase):
    ...
    def test_tweets_status_code(self):
        # sends HTTP GET request to the application on the specified path
        result = self.app.get('/api/v1/tweets')
        # assert the status code of the response
        self.assertEqual(result.status_code, 200)

    def test_addusers_status_code(self):
        # sends HTTP POST request to the application on the specified path
    ...
    def test_updusers_status_code(self):
        ...
        result = self.app.put('/api/v1/users/5', data='{"username":"Ovestint", "email": "leolaLguertin@teleworm.us"}', content_type='application/json')
        # assert the status code of the response
        self.assertEquals(result.status_code, 200)

    def test_addtweets_status_code(self):
        # sends HTTP GET request to the application on the specified path
        result = self.app.post('/api/v1/tweets', data='{"username":"user1", "body": "It Works!#Awesome"}', content_type='application/json')
        # assert the status code of the response
        self.assertEqual(result.status_code, 201)

    def test_delusers_status_code(self):
        # sends HTTP Delete request to the application on the specified path
